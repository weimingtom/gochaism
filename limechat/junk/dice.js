/**
 * dice.js for LimeChat
 * BisqBotの .dice の簡易版
 * 
 * スクリプトを動作させたいチャンネルで
 *  dice/start
 * と発言するか、あるいは
 *  dice/start #channel1 #channel2 ...
...
function dice(prefix, target, count, sides)
{
    var maxcount = 20;
    var rn, sum;
    var msg, an, results;
    ...
    if (count > maxcount) {
        textOut(target, scriptName + ' : ' + prefix.nick + 'さん、それ回数多すぎっ (＞д＜)');
        return;
    }
    ...
    if (sides > 911926508163698167129671532981361600001000001) {
        textOut(target, prefix.nick + 'さんがブラックホールをころころして宇宙消滅 (＞д＜) BGM:盆回り');
        return;
    }
    else if (sides > 91192650816300001) {
        textOut(target, prefix.nick + 'さんがブラックホールをころころして銀河系消滅 (＞д＜) BGM:盆回り');
        return;
    }
    else if (sides > 711926508163) {
        textOut(target, prefix.nick + 'さんがブラックホールをころころして地球消滅 (＞д＜) BGM:盆回り');
        return;
    }
    else if (sides > 12000000) {
        textOut(target, prefix.nick + 'さんが' + sides + '面サイコロを……持てないみたい (・д・)');
        return;
    }
    else if (sides > 300000) {
        textOut(target, prefix.nick + 'さんが' + an + sides + '面サイコロをころころしたよ。でも結果がよく見えないや (・д・)');
        return;
    }
    else if (sides == 1) {
        textOut(target, prefix.nick + 'たんがおまめさんをころころしてあそんでるよ！ (〃v〃)');
        return;
    }
    else if (count == 0) {
        return;
        ...
        textOut(target, prefix.nick + '、なにかんがえてるの？ (・д・)');
        return;
    }
    ...
    if (sides == 2)
        msg = prefix.nick + 'さんがコイン' + (count > 1 ? count + '枚' : '') + '投げたよ: ' + results + '';
    else
...
function onTextPost(prefix, target, text)
{
    ...
    if (text.match(/^\.dice\s+([+-]?\d+)d([+-]?\d+)\s*/))
        dice(prefix, target, parseInt(RegExp.$1), parseInt(RegExp.$2));
    else if (text.match(/^\.dice(\s+.*|)$/))
        textOut(target, ".dice <count>d<sides>");
}
