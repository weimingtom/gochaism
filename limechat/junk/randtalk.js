/**
 * randtalk.js for LimeChat
 * ランダムになにか発言するスクリプト
 * 
 * スクリプトを動作させたいチャンネルで
 *  randtalk/start
 * と発言するか、あるいは
 *  randtalk/start #channel1 #channel2 ...
...
var talkInterval = 240;
var talkProbability = 0.04;
...
function randtalk()
{
    ...
    for (key in chList) {
        ...
        for (ch = 0; ch < channels.length; ch++) {
            if (channels[ch].active && channels[ch].name.toLowerCase() == key) {
                chFound = true;
                break;
            }
        }
    ...
    chIndex = randomInt(chListLength - 1);
    for (key in chList) {
        if (!chList[key])
            continue;
    ...
    if (Math.random() < talkProbability) {
        talkOut(target, "にゃんにゃん！ (＞ヮ＜)");
        debugOut(target, "randtalk: <" + target + "> にゃんにゃん！ (＞ヮ＜)");
    }
}

function setTalkTimer()
{
    setInterval(randtalk, talkInterval * 1000);
}
